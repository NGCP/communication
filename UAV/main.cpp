...
namespace message {
	...
	void to_connectjson(json& j, const connect& msg) {
		to_json(j, (message)msg);
		j.push_back({ "jobsAvailable", msg.jobsAvailable });
	}

	void from_connectjson(const json& j, connect& msg) {
		from_json(j, msg);
	...
	void to_connectackjson(json& j, const connectionAck& msg) {
		to_json(j, (message)msg);
	}

	void from_connectackjson(const json& j, connectionAck& msg) {
		from_json(j, msg);
	}
}
...
int main() {
	...
	// 2. Create JSON
	json connect_json;
	message::to_connectjson(connect_json, connect_msg);
	...
	message::from_connectackjson(connectionack_json, connectionack_msg);
	std::cout << "CONNECTIONACK STRUCT" << std::endl;
	std::cout << connectionack_msg.type << std::endl;
	printf("id: %ld\n", connectionack_msg.id);
	printf("sid: %ld\n", connectionack_msg.sid);
	printf("tid: %ld\n", connectionack_msg.tid);
	printf("time: %lld\n", connectionack_msg.time);
}
